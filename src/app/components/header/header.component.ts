...
export class HeaderComponent implements OnInit {
  ...
  submitForm(): void {
    this.hotelsCardsServiceService.AllHotelsTogether = []
    setTimeout(() => {
      this.hotelsCardsServiceService.AllHotelsTogether = this.hotelsCardsServiceService.AllHotelsTogetherAuxList.filter((e) => e.address.city === this.hotelsCardsServiceService.city) &&
        this.hotelsCardsServiceService.AllHotelsTogetherAuxList.filter((e) => e.name.toLowerCase().includes(this.hotelsCardsServiceService.city.toLowerCase()));
      this.checkIfTheListIsEmpty();
    }, 1500)
  }

  checkIfTheListIsEmpty(): void {
    if (this.hotelsCardsServiceService.AllHotelsTogether.length <= 0)
